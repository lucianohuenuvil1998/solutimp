...
$(document).ready(function () {
  ...
  // });
    ...
    // Implementación js para obtener rut del usuario en el formulario - Luciano

  $(document).on('click', '.btn.btn-search-account', function (e) {
    e.preventDefault();
    let customerController = $(this).attr('data-customer-controller');
    // Extraer RUT
    let rut = $('#customer_rut').val();
    if (rut) {
        // Primera búsqueda: buscar por RUT en direcciones
        $.ajax({
          type: 'GET',
          url: quotationspro_link,
          data: {
              ajax: '1',
              action: 'searchByRut',  // Nombre de la nueva función
              rut: rut  // El valor del RUT que estás buscando
          },

          success: function (data) {
              if (typeof data === 'string') {
                data = JSON.parse(data); // Parsear si es una cadena
              }

              if (data.success && data.customer) {
                  console.log("el usuario se ha encontrato", data.customer)

                  emailRut = data.customer.email.toString();
                  nameRut = data.customer.firstname.toString();
                  lastNameRut = data.customer.lastname.toString();

                  realizarBusqueda(nameRut,lastNameRut,emailRut, customerController);
              } else {
                  alert('Cliente no encontrado por RUT.');
              }
            },
        });
    } else {
      ...
      realizarBusqueda(firstname,lastname,email,customerController);
    }
  });


  function realizarBusqueda(firstname,lastname,email, customerController){
    if (email ) {
      var search = email;
    } else {
      var search = [firstname, lastname].filter(Boolean).join(' ');
