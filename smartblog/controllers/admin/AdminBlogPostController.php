...
class AdminBlogPostController extends ModuleAdminController
{
	...
	public function renderForm()
	{
		...
		$this->fields_form = array(
			...
			'input'  => array(
				...
				array(
					'type'     => 'textarea',
					'label'    => $this->l('Meta Description (90 Characters)'),
		...
		if(Module::isInstalled('smartblogrelatedcategory') && Module::isEnabled('smartblogrelatedcategory')) {
			$selectedcatgs = $this->getSelectedCategory($id_smart_blog_post);
			$selectedpricess = $this->getSelectedCategoryPrices($id_smart_blog_post);
			
			$this->fields_value['relatedcategorys'] = $selectedcatgs;
			if(isset($selectedpricess['related_min_price'])){
				$this->fields_value['relatedcategorysmin'] = $selectedpricess['related_min_price'];
			}else{
				$this->fields_value['relatedcategorysmin'] = '';
			}

			if(isset($selectedpricess['related_max_price'])){
				$this->fields_value['relatedcategorysmax'] = $selectedpricess['related_max_price'];
			}else{
		...
		$sql      = 'SELECT count(`id_smart_blog_post`) FROM `' . _DB_PREFIX_ . 'smart_blog_post`';
		$count_content = DB::getInstance()->getValue( $sql );
		if($count_content >= 2){
			$id_smart_blog_post = Tools::getValue('id_smart_blog_post');
			if(!$id_smart_blog_post){
				$html = "";
				$html.='<div class="panel col-lg-12">
				<div class="font-prev-wrapper" style="text-align: center;">
				<h2>You Need The Pro Version to Add More Than Seven Blog Posts. <a href="https://classydevs.com/free-modules/smartblog/?utm_source=sbfree_backoffice&utm_medium=sbfree_backoffice&utm_campaign=sbfree_backoffice&utm_id=sbfree_backoffice&utm_term=sbfree_backoffice&utm_content=sbfree_backoffice#sb_price">Get SmartBlog Pro</a></div></div>';
				return $html;
			}else{
	...
	private function getSelectedCategoryPrices($id_blog){
		$sql = 'SELECT DISTINCT(`id_smart_blog_post`),`related_min_price`,`related_max_price` FROM `' . _DB_PREFIX_ . 'smart_blog_category_related` WHERE `id_smart_blog_post` = "'.$id_blog.'"';
	...
	protected function afterAdd($object)
	{
		...
		if (!empty($id)) {
			...
			foreach($object->meta_description as $key => $description){
				$object->meta_description[$key] = substr($description, 0, 90);
			}
	...
	protected function afterUpdate($object)
	{
		...
		if (!empty($id)) {
			...
			if (version_compare(_MODULE_SMARTBLOG_VERSION_, '4.1.1', '>')) {
				foreach($object->meta_description as $key => $description){
					$object->meta_description[$key] = substr($description, 0, 90);
				}
			}
