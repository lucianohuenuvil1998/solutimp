...
class AdminBlogCategoryController extends AdminController {
	...
	public function renderForm() {
		...
		$sql      = 'SELECT count(`id_smart_blog_category`) FROM `' . _DB_PREFIX_ . 'smart_blog_category`';
		$count_content = DB::getInstance()->getValue( $sql );
		if($count_content >= 3){
			$id_smart_blog_category = Tools::getValue('id_smart_blog_category');
			if(!$id_smart_blog_category){
				$html = "";
				$html.='<div class="panel col-lg-12">
				<div class="font-prev-wrapper" style="text-align: center;">
				<h2>You Need The Pro Version to Add More Than Three Blog Categories.  <a href="https://classydevs.com/free-modules/smartblog/?utm_source=sbfree_backoffice&utm_medium=sbfree_backoffice&utm_campaign=sbfree_backoffice&utm_id=sbfree_backoffice&utm_term=sbfree_backoffice&utm_content=sbfree_backoffice#sb_price">Get SmartBlog Pro</a></div></div>';
				return $html;
			}else{
