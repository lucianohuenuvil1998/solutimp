...
define('_MODULE_SMARTBLOG_VERSION_', '4.1.4');
...
define('_MODULE_SMARTBLOG_ROOT_DIR_', _PS_MODULE_DIR_ . 'smartblog/');
...
class smartblog extends Module
{
	...
	public function __construct()
	{
		...
		$this->description      = $this->trans('The Most Powerfull Prestashop Blog  Module - by ClassyDevs', [], 'Modules.Smartblog.Smartblog');
	...
	public function install()
	{
		...
		Configuration::updateGlobalValue('smarttheme', 'default');
		...
		Configuration::updateGlobalValue('smartblogmetadescrip', 'Prestashop powerfull blog site developing module. It has hundrade of extra plugins. This module developed by classydevs.com');
	...
	public function hookDisplayBackOfficeHeader($params)
	{
		...
		if($controller_name == 'AdminBlogPost'){
			if ( \Module::isInstalled( 'crazyelements' ) && \Module::isEnabled( 'crazyelements' ) ) {
				$has_crazy = "1";
			}else{
		...
		$this->smarty->assign(
			array(
				...
				'icon_url'     => _MODULE_SMARTBLOG_IMAGE_URL_ . '/logo-icon.svg',
				'hascrazy' => $has_crazy
			)
		);
            ...
            <div class="dashboard-presents-product-container">
                <div class="dashboard-presents-product-bottom-content">
                    ...
                    <div class="dashboar-single-presents-item db-second-item">
                        <a href="https://classydevs.com/free-modules/smartblog/?utm_source=sbfree_backoffice&utm_medium=sbfree_backoffice&utm_campaign=sbfree_backoffice&utm_id=sbfree_backoffice&utm_term=sbfree_backoffice&utm_content=sbfree_backoffice#sb_price"
                            target="_blank">
                            <img src="<?php echo _MODULE_SMARTBLOG_IMAGE_URL_ . 'sb-inner-pricing.png'; ?>" alt="Logo">
                        </a>
                    </div>
                </div>
            </div>
...
<?php
	...
	public function uninstall()
	{
		if (
			...
			|| !Configuration::deleteByName('smarttheme')
	...
	public function getContent()
	{
		...
		$config_form = $this->display(__FILE__, 'views/templates/admin/config.tpl');
		if (Tools::isSubmit('savesmartblog')) {
			...
			Configuration::updateValue('smarttheme', Tools::getvalue('smarttheme'));
			...
			if (!$purchase_code || empty($purchase_code)) {
				$html .= $this->displayError($this->trans('Please Enter a Purchase Code', [], 'Modules.Smartblog.Smartblog'));
			} else {

				include_once _MODULE_SMARTBLOG_CLASS_DIR_ . 'SmartBlogLicense.php';
				$license = new SmartBlogLicense(1, false);
				if ($action) {
					...
					if ($status) {
						...
						$html .= $this->displayConfirmation($this->trans('Purchase Code Activated', [], 'Modules.Smartblog.Smartblog'));
					} else {
						Configuration::updateValue('SMARTBLOG_LICENSE', '');
						Configuration::updateValue('SMARTBLOG_LICENSE_VALIDITY', 'invalid');
						$html .= $this->displayError($this->trans('Invalid Purchase Code. Please try again!!!', [], 'Modules.Smartblog.Smartblog'));
					}
				} else {
					...
					if ($status) {
						...
						$html .= $this->displayConfirmation($this->trans('Purchase Code Dectivated', [], 'Modules.Smartblog.Smartblog'));
					} else {
						Configuration::updateValue('SMARTBLOG_LICENSE', '');
						Configuration::updateValue('SMARTBLOG_LICENSE_VALIDITY', 'invalid');
						$html .= $this->displayError($this->trans('Invalid Purchase Code. Please try again!!!', [], 'Modules.Smartblog.Smartblog'));
	...
	protected function activationForm()
	{
		...
		if ($validity == 'valid') {
			...
			if(!isset($stable) || $stable == ''){
				$stable = _MODULE_SMARTBLOG_VERSION_;
			}
			$info_msg = '<div class="col-lg-9 module-info free-annouunce">You Are Using SmartBlog Free Version!!!</div>
			<div class="col-lg-9 module-info"> Installed Version : ' . _MODULE_SMARTBLOG_VERSION_ . '</div>
				<div class="col-lg-9  module-info"> Available Version : ' . $stable . '</div>
				<div class="col-lg-9  module-info"> <a href="https://classydevs.com/docs/smartblog/?utm_source=sbbackoffice&utm_medium=sbbackoffice&utm_campaign=sbbackoffice&utm_id=sbbackoffice&utm_term=sbbackoffice&utm_content=sbbackoffice" target="_blank">Check Documentation</a></div>
				<div class="col-lg-9  module-info promsg"> To Get Support <a href="https://classydevs.com/free-modules/smartblog/?utm_source=sbfree_backoffice&utm_medium=sbfree_backoffice&utm_campaign=sbfree_backoffice&utm_id=sbfree_backoffice&utm_term=sbfree_backoffice&utm_content=sbfree_backoffice#sb_price" target="_blank">Get SmartBlog Pro</a></div>';
		}
		
		
		$this->fields_form[0]['form'] = array(
			'legend' => array(
				'title' => $this->trans('Module Activation', [], 'Modules.Smartblog.Smartblog'),
			),
			'input'  => array(
				array(
					'type'     => 'text',
					'label'    => $this->trans('Purchase Code', [], 'Modules.Smartblog.Smartblog'),
				...
				array(
					'type'     => 'html',
					'name'     => 'smartbloginfohtml',
					'html_content'    => $info_msg
				),
				array(
					'type'     => 'hidden',
					'name'     => 'licenseaction',
	...
	public function SettingForm()
	{
		...
		$this->fields_form[0]['form'] = array(
			...
			'input'  => array(
				...
				array(
					'type'     => 'textarea',
					'label'    => $this->trans('Meta Description (90 Characters)', [], 'Modules.Smartblog.Smartblog'),
					'name'     => 'smartblogmetadescrip',
					'rows'     => 7,
					'maxlength'=> 90,
					'cols'     => 66,
					'required' => true,
					'desc'     => '<p class="alert alert-warning"><a href="https://classydevs.com/free-modules/smartblog/?utm_source=sbfree_backoffice&utm_medium=sbfree_backoffice&utm_campaign=sbfree_backoffice&utm_id=sbfree_backoffice&utm_term=sbfree_backoffice&utm_content=sbfree_backoffice#sb_price">Get SmartBlog Pro</a> to Be Able to Add Meta Description Unlimited Length</p>'
				),
				array(
					'type'     => 'html',
					'name'     => 'smartblogprohtml',
					'html_content'    => '<p class="alert alert-info"><a href="' . $blog_url . '">' . $blog_url . '</a></p>'
				),
				array(
					'type'     => 'html',
					'name'     => 'smartblogprohtml',
					'html_content'    => '<div class="col-lg-9 module-info promsg"> To Change URL Pattern to SEO Friendly URL Pattern and Stucture <a href="https://classydevs.com/free-modules/smartblog/?utm_source=sbfree_backoffice&utm_medium=sbfree_backoffice&utm_campaign=sbfree_backoffice&utm_id=sbfree_backoffice&utm_term=sbfree_backoffice&utm_content=sbfree_backoffice#sb_price" target="_blank">Click Here to Get PRO</a></div>'
				),
				array(
					'type'     => 'text',
				...
				array(
					...
					'size'     => 15,
					'required' => true,
					'desc'     => '<p class="alert alert-info"><a target="_blank" href="https://classydevs.com/docs/smartblog/">Documentation on date and time formatting</a></p>',
				),
				array(
					'type'     => 'select',
					'label'    => $this->trans('Theme', [], 'Modules.Smartblog.Smartblog'),
					'name'     => 'smarttheme',
					'options' => [
						'query' => $this->smartblogthemelist(),
						'id' => 'val',
						'name' => 'lab',
					],
				),
				...
				array(
					'type'     => 'html',
					'name'     => 'smartblogprohtml',
					'html_content'    => '<div class="col-lg-9 module-info promsg"> To Enable Captcha <a href="https://classydevs.com/free-modules/smartblog/?utm_source=sbfree_backoffice&utm_medium=sbfree_backoffice&utm_campaign=sbfree_backoffice&utm_id=sbfree_backoffice&utm_term=sbfree_backoffice&utm_content=sbfree_backoffice#sb_price" target="_blank">Click Here to Get PRO</a></div>'
				),
				array(
					'type'     => 'switch',
				...
				array(
					'type'     => 'html',
					'name'     => 'smartblogprohtml',
					'html_content'    => '<div class="col-lg-9 module-info promsg"> To Select Author Name Style <a href="https://classydevs.com/free-modules/smartblog/?utm_source=sbfree_backoffice&utm_medium=sbfree_backoffice&utm_campaign=sbfree_backoffice&utm_id=sbfree_backoffice&utm_term=sbfree_backoffice&utm_content=sbfree_backoffice#sb_price" target="_blank">Click Here to Get PRO</a></div>'
				),
				array(
					'type'          => 'file',
				...
				array(
					'type'     => 'html',
					'name'     => 'smartblogprohtml',
					'html_content'    => '<div class="col-lg-9 module-info promsg"> To Add Custom Css <a href="https://classydevs.com/free-modules/smartblog/?utm_source=sbfree_backoffice&utm_medium=sbfree_backoffice&utm_campaign=sbfree_backoffice&utm_id=sbfree_backoffice&utm_term=sbfree_backoffice&utm_content=sbfree_backoffice#sb_price" target="_blank">Click Here to Get PRO</a></div>'
				)
			),
			'submit' => array(
		...
		$helper->fields_value['smarttheme']         	 = Configuration::get('smarttheme');
